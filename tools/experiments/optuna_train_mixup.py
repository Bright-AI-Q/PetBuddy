"""
Optuna hyperparameter optimization training script for mixup parameters only.
Note that this assumes that the core parameters are already optimized.
To optimize both the core training parameter and mixup together, use optuna_train_all.py script
...
def objective(trial: optuna.trial.Trial):
    ...
    # Use frozen values from config
    lr = petnet_config["train"]["learning_rate"]
    weight_decay = petnet_config["train"]["weight_decay"]
    optimizer_name = petnet_config["train"]["optimizer"]
    ...
    print(
        f"🚀 Starting trial {trial.number}: mixup_alpha={mixup_alpha:.2e}, cutmix_alpha={cutmix_alpha:.2e}, mixup_prob={mixup_prob:.2e}, switch_prob:{switch_prob:.2e}, label_smoothing:{label_smoothing:.2e} "
    )
...
def main():
    ...
    study = optuna.create_study(
        ...
        direction="maximize",
        pruner=pruner,
        storage="sqlite:///petnet_augment_study.db",
        load_if_exists=True,
    )

    print("🔬 Starting Optuna hyperparameter optimization (Phase 2: Mixup parameters)")
    print(
        "📊 Optimizing: Mixup alpha, cutmix alpha, mixup prob, switch prob, label smoothing"
    )
